    ...
    contract Redenom is ERC20Interface, Owned{
        ...
        uint _K; // coef to count last divident on full tokens
        ...
        function redenominate() public onlyAdmin returns(uint current_round){
            ...
            if(round<8){ // if round between 1 and 7 
                ...
                if(round==8){
                    ...
                    //Вычисляем коэфф для расчета последней выплаты
                    //Затем используем его при выдаче в updateAccount 
                    //C помощью count_last_div (считает сколько дать).
